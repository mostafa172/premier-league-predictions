...
export class PredictionsController {
  ...
  async createPrediction(req: AuthenticatedRequest, res: Response) {
    try {
      ...
      if (isDouble === true) {
        ...
        const unlockedIds = sameGwFixtures
          .filter((f) => !isFixtureLocked(f))
          .map((f) => f.id);
        if (unlockedIds.length > 0) {
          await Prediction.update(
            { isDouble: false },
            { where: { userId, fixtureId: { [Op.in]: unlockedIds } } }
          );
        }
      }
  ...
  async getUserGameweekTotal(req: AuthenticatedRequest, res: Response) {
    try {
      ...
      const totalPoints =
        results && (results as any).totalPoints
          ? parseInt((results as any).totalPoints, 10)
          : 0;
